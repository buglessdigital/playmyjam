...
export default function NewVenueForm() {
  ...
  // Mekan talebi ekranındaki "Mekan Oluştur" kısayolu adı ?name= ile taşır;
  // slug ve kullanıcı adı bu addan türetilir (bkz. handleNameChange)
  const prefillName = useSearchParams().get("name")?.slice(0, 80) ?? "";

  const [name, setName] = useState(prefillName);
  const [slug, setSlug] = useState(prefillName ? toSlug(prefillName) : "");
